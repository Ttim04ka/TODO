...
const plainedReducer=(state=initialState, action)=>{
    switch (action.type) {
      ...
      case 'DELETE_TASK':
        let newArr=state.tasks.filter(oldTask=>oldTask!==action.task);
        let newCompletedTasksArr=state.completedTasks.filter(oldTask=>oldTask.current!==action.task);
        return {...state,tasks:newArr,completedTasks:newCompletedTasksArr};
